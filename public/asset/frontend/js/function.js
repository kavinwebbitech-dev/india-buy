(function ($) {
  ...
  if ($(".project-company-slider").length) {
    const project_company_slider = new Swiper(
  ...
  if ($(".testimonial-slider-elite").length) {
    const testimonial_slider_elite = new Swiper(
  ...
  if ($(".skills-progress-bar-metal").length) {
    $(".skills-progress-bar-metal").waypoint(
      function () {
        $(".skillbar-metal").each(function () {
          $(this)
            .find(".count-bar-metal")
            .animate(
              {
                width: $(this).attr("data-percent"),
              },
...
function updateLightboxContent(index) {
  ...
  // Uses data-full if present, otherwise falls back to src
  activeImg.src = img.getAttribute("data-full") || img.src;
  activeTitle.innerText = title;
  activeSub.innerText = sub;
}
...
// Controls
document.querySelector(".door-prev").onclick = (e) => {
  e.stopPropagation();
  moveSlide(-1);
};
document.querySelector(".door-next").onclick = (e) => {
  e.stopPropagation();
  moveSlide(1);
};
document.querySelector(".door-close").onclick = () => {
