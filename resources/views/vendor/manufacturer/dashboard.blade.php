        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <main class="flex-1">
                        ...
                        <table class="w-full text-left border-collapse min-w-[800px]">
                            <thead>
                                <tr
                                    class="bg-white border-b border-gray-100 text-[12px] uppercase tracking-wider text-gray-400 font-bold">
                                    <th class="px-6 py-4 w-12 text-center">#</th>
                                    <th class="px-6 py-4">Sent To (Supplier / User)</th>
                                    <th class="px-6 py-4">Inquiry Subject / Product</th>
                                ...
                                @forelse ($quotations as $index => $quote)
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors bg-white group">
                                        ...
                                        <td class="px-6 py-5">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    ...
                                                    <img src="{{ $image }}"
                                                        class="w-full h-full object-contain" alt="item">
                                                </div>
                                                <div>
                                                    <p
                                                        class="font-medium text-gray-900 line-clamp-1 max-w-[220px] text-[13px]">
                                        ...
                                        <td class="px-6 py-5 text-center flex items-center justify-center gap-2">
                                            <a href="#"
                                                onclick="openChat({{ $enquiry->id ?? 0 }}, '{{ addslashes($title) }}', '{{ addslashes($enquiry->receiver->name ?? 'Supplier') }}', '{{ $image }}')"
                                                class="inline-flex items-center gap-2 bg-slate-100 hover:bg-slate-200 px-3 py-2 rounded-xl text-[13px] font-bold transition-colors">
                                                <i data-lucide="eye" class="w-4 h-4"></i> View
                                            </a>

                                            @if ($quote->file_path)
                                                <a href="{{ asset($quote->file_path) }}" target="_blank"
                                                    class="inline-flex items-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 px-3 py-2 rounded-xl text-[13px] font-bold transition-colors">
                ...
                <div
                    class="border-t border-gray-100 p-4 bg-white flex items-center justify-between text-[13px] text-gray-500">
                    <span>
                        Showing {{ $quotations->firstItem() ?? 0 }} to {{ $quotations->lastItem() ?? 0 }} of
                        {{ $quotations->total() }} entries
                    </span>
                    <div>
                        {{ $quotations->links() }}
                    </div>
