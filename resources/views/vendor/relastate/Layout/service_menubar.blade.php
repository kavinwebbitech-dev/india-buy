...
<div class="bg-white rounded-2xl border border-gray-100 p-2 mb-8 flex items-center gap-2 overflow-x-auto shadow-sm">
    ...
    {{-- PRODUCTS --}}
    <a href="{{ route('relastate.product.list') }}"
        class="px-5 py-2.5 rounded-xl text-[14px] font-medium whitespace-nowrap
        {{ request()->routeIs('relastate.product.*')
            ? 'bg-primary text-white'
            : 'text-gray-500 hover:bg-slate-50' }}">
    ...
    {{-- PROFILE --}}
    <a href="{{ route('relastate.profile') }}"
        class="px-5 py-2.5 rounded-xl text-[14px] font-medium whitespace-nowrap
        {{ request()->routeIs('relastate.profile') || request()->routeIs('relastate.profile.update')
            ? 'bg-primary text-white'
            : 'text-gray-500 hover:bg-slate-50' }}">
